...
void MonitorTestTool::saveSamplingSettings()
{
	...
	if (curMin == minY && curMax == maxY && curNum == samplingNum) return;	
	if (curMin >= curMax) {
...
QGroupBox * MonitorTestTool::createSamplingGroup()
{
	...
	auto maxSizeLabel = new QLabel(tr("采集个数: "), this);
